...
-- Consulta 1: Ver encomendas pendentes para a unidade 'A-101' (JOIN)
SELECT
    e.descricao AS 'Encomenda',
    e.data_recebimento,
    p_func.nome AS 'Registrado por'
FROM Encomenda e
JOIN Unidade u ON e.id_unidade_destino = u.id_unidade
JOIN Pessoa p_func ON e.id_pessoa_funcionario = p_func.id_pessoa
WHERE
    u.bloco = 'A'
    AND u.numero_apto = '101'
    AND e.status = 'AGUARDANDO';


-- Consulta 2: Ver todos os pets e seus donos (JOIN)
SELECT
    p.nome_pet AS 'Pet',
    p.especie,
    p.status,
    p_morador.nome AS 'Dono',
    p_morador.telefone AS 'Contato Dono'
FROM Pet p
JOIN Pessoa p_morador ON p.id_pessoa_morador = p_morador.id_pessoa;


-- Consulta 3: Ver histórico de visitas de um visitante específico (JOIN Triplo)
SELECT
    v.nome_completo AS 'Visitante',
    r.data_hora_entrada,
    r.data_hora_saida,
    CONCAT(u.bloco, '-', u.numero_apto) AS 'Unidade Visitada'
FROM RegistroEntrada r
JOIN Visitante v ON r.id_visitante = v.id_visitante
JOIN Unidade u ON r.id_unidade_visitada = u.id_unidade
WHERE
    v.nome_completo = 'Pedro Alves';